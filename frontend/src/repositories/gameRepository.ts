...
export interface GameRepository {
  ...
  guess(gameId: string, roundId: string, year: number, coordinates: Coordinates, timedOut?: boolean, hintIds?: string[]): Promise<Game>
...
export class HttpGameRepository implements GameRepository {
  ...
  guess(gameId: string, roundId: string, year: number, coordinates: Coordinates, timedOut = false, hintIds: string[] = []): Promise<Game> {
    return this.request(`/games/${gameId}/rounds/${roundId}/guess`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ year, lat: coordinates.lat, lng: coordinates.lng, timedOut, hintIds }),
    })
  }
