...
export class GameService {
  ...
  guess(game: Game, year: number, coordinates: Coordinates, timedOut = false, hintIds: string[] = []): Promise<Game> {
    if (!game.currentRound) throw new Error('Раунд не найден')
    return this.repository.guess(game.id, game.currentRound.id, year, coordinates, timedOut, hintIds)
  }
